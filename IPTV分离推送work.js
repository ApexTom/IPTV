...
async function handleUpdate(request, env) {
  ...
  for (let i = 0; i < fetchResults.length; i++) {
    ...
    if (result.status === "rejected" || !result.value) {
      console.log("Source failed:", sourceUrl)
      failedCount++
      sourceStats.push({ keyword, ok: false, channels: 0, groups: 0, groupKeys: [] })
      continue
    }

    const { groups, channelCount } = parseM3U(result.value)
  ...
  for (const stat of sourceStats) {
    if (!stat.ok) {
      sourceDetail += `❌ ${stat.keyword}  拉取失败\n\n`
      continue
    }
